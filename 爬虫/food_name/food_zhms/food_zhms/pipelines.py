...
# worksheet = workbook.add_sheet('zhms', cell_overwrite_ok=True)
# class FoodZhmsPipeline(object):
...
class FoodZhmsPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if valid:
            newfoodurllist=[]
            for f in item['foodurl']:
                f="http://zhms.cn" + f
                newfoodurllist.append(f)
            item['foodurl']=newfoodurllist
